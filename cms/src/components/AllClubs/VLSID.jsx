...
useEffect(() => {
  ...
  const fetchClubDataAndEvents = async () => {
    if (userRole === "faculty") {
      ...
      if (selectedClubs.includes("VLSID")) {
        setIsLeadForVLSID(true);
      }
    } else if (
      (userRole === "lead" && userClub === "VLSID") ||
      userRole === "admin"
    ) {
      setIsLeadForVLSID(true);
    }
...
const fetchFacultyClubs = async (email) => {
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/clubs/VLSID");
      if (response.data) {
  ...
  const handleLogoUpload = async (event) => {
    ...
    formData.append('clubName', "VLSID"); // Include the club name
  ...
  const renderEventCard = (event) => {
    ...
    const canViewProfiles = userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForVLSID;
    const isUpcoming = new Date(event.date) >= new Date();
    const canUploadDocument = !isUpcoming && (userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForVLSID);
