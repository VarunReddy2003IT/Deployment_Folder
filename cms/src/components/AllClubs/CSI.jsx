...
useEffect(() => {
  ...
  const fetchClubDataAndEvents = async () => {
    if (userRole === "faculty") {
      ...
      if (selectedClubs.includes("CSI")) {
        setIsLeadForCSI(true);
      }
    } else if (
      (userRole === "lead" && userClub === "CSI") ||
      userRole === "admin"
    ) {
      setIsLeadForCSI(true);
    }
...
const fetchFacultyClubs = async (email) => {
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/clubs/CSI");
      if (response.data) {
  ...
  const handleLogoUpload = async (event) => {
    ...
    formData.append('clubName', "CSI"); // Include the club name
  ...
  const renderEventCard = (event) => {
    ...
    const canViewProfiles = userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForCSI;
    const isUpcoming = new Date(event.date) >= new Date();
    const canUploadDocument = !isUpcoming && (userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForCSI);
