...
useEffect(() => {
  ...
  const fetchClubDataAndEvents = async () => {
    if (userRole === "faculty") {
      ...
      if (selectedClubs.includes("NSS2")) {
        setIsLeadForNSS2(true);
      }
    } else if (
      (userRole === "lead" && userClub === "NSS2") ||
      userRole === "admin"
    ) {
      setIsLeadForNSS2(true);
    }
...
const fetchFacultyClubs = async (email) => {
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/clubs/NSS2");
      if (response.data) {
  ...
  const handleLogoUpload = async (event) => {
    ...
    formData.append('clubName', "NSS2"); // Include the club name
  ...
  const renderEventCard = (event) => {
    ...
    const canViewProfiles = userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForNSS2;
    const isUpcoming = new Date(event.date) >= new Date();
    const canUploadDocument = !isUpcoming && (userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForNSS2);
