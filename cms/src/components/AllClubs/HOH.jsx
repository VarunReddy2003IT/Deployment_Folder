...
useEffect(() => {
  ...
  const fetchClubDataAndEvents = async () => {
    if (userRole === "faculty") {
      ...
      if (selectedClubs.includes("HOH")) {
        setIsLeadForHOH(true);
      }
    } else if (
      (userRole === "lead" && userClub === "HOH") ||
      userRole === "admin"
    ) {
      setIsLeadForHOH(true);
    }
...
const fetchFacultyClubs = async (email) => {
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/clubs/HOH");
      if (response.data) {
  ...
  const handleLogoUpload = async (event) => {
    ...
    formData.append('clubName', "HOH"); // Include the club name
  ...
  const renderEventCard = (event) => {
    ...
    const canViewProfiles = userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForHOH;
    const isUpcoming = new Date(event.date) >= new Date();
    const canUploadDocument = !isUpcoming && (userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForHOH);
      ...
      <div className="content">
        <div className="page-content">
          ...
          <div className="club-header">
            ...
            <h1 className="page-title">HOH</h1>
          </div>
