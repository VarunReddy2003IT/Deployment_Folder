...
useEffect(() => {
  ...
  const fetchClubDataAndEvents = async () => {
    if (userRole === "faculty") {
      ...
      if (selectedClubs.includes("Rotract")) {
        setIsLeadForRotract(true);
      }
    } else if (
      (userRole === "lead" && userClub === "Rotract") ||
      userRole === "admin"
    ) {
      setIsLeadForRotract(true);
    }
...
const fetchFacultyClubs = async (email) => {
  ...
  const fetchClubData = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/clubs/Rotract");
      if (response.data) {
  ...
  const handleLogoUpload = async (event) => {
    ...
    formData.append('clubName', "Rotract"); // Include the club name
  ...
  const renderEventCard = (event) => {
    ...
    const canViewProfiles = userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForRotract;
    const isUpcoming = new Date(event.date) >= new Date();
    const canUploadDocument = !isUpcoming && (userRole === 'admin' || (userRole === 'lead' && userClub === event.club)||isLeadForRotract);
